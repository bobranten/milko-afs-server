...
if test "$crypto_lib" = "unknown" -a "$with_krb4" != "no"; then
	...
	for i in $INCLUDE_krb4; do
		...
		for j in $cdirs; do
			for k in $clibs; do
				LIBS="$j $k $save_LIBS"
				AC_LINK_IFELSE([AC_LANG_PROGRAM([[test_headers]], [[test_body]])],[openssl=yes ires="$i" lres="$j $k"; break 3],[])
			done
		done
		...
		for j in $cdirs; do
			for k in $clibs; do
				LIBS="$j $k $save_LIBS"
				AC_LINK_IFELSE([AC_LANG_PROGRAM([[test_headers]], [[test_body]])],[openssl=no ires="$i" lres="$j $k"; break 3],[])
			done
		done
