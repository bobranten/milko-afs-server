dnl
dnl $Id: bsd-vget.m4,v 1.6 2004/07/20 11:23:35 lha Exp $
dnl
...
AC_CACHE_CHECK(if vget takes four arguments, ac_cv_func_vget_four_args,
AC_TRY_COMPILE_KERNEL([
#ifdef HAVE_SYS_CDEFS_H
#include <sys/cdefs.h>
#endif
...
ac_cv_func_vget_four_args=yes,
ac_cv_func_vget_four_args=no))
if test "$ac_cv_func_vget_four_args" = yes; then
	AC_DEFINE(HAVE_FOUR_ARGUMENT_VGET, 1,
	[define if vget takes four arguments])
fi

if test "$ac_cv_func_vget_one_args" = "no" -a "$ac_cv_func_vget_two_args" = "no" -a "$ac_cv_func_vget_three_args" = "no" -a "$ac_cv_func_vget_four_args" = "no"; then
