...
save_CFLAGS="${CFLAGS}"
AC_CACHE_CHECK([for IPv6 stack type], ac_cv_v6type,
[dnl check for different v6 implementations (by itojun)
ac_cv_v6type=unknown
v6lib=none
...
yes
#endif],
			[ac_cv_v6type=$i; CFLAGS="-DINET6 $CFLAGS"])
		;;
	zeta)
		AC_EGREP_CPP(yes, [
#include <sys/param.h>
...
yes
	...
	if test "$ac_cv_v6type" != "unknown"; then
		break
	fi
done
...
]])],[ac_cv_lib_ipv6=yes],[ac_cv_lib_ipv6=no])])
