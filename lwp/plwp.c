...
Dump_One_Process(PROCESS pid)
{
    ...
    printf("***LWP: Process Control Block at 0x%lx\n", (long)pid);
    printf("***LWP: Name: %s\n", pid->name);
    if (pid->ep != NULL)
	printf("***LWP: Initial entry point: 0x%lx\n", (long)pid->ep);
    ...
    putchar('\n');
    printf("***LWP: Priority: %d \tInitial parameter: 0x%lx\n",
	   pid->priority, (long)pid->parm);

    if (pid->eventcnt > 0) {
	printf("***LWP: Number of events outstanding: %d\n", pid->waitcnt);
	printf("***LWP: Event id list:");
	for (i=0;i<pid->eventcnt;i++)
	    printf(" 0x%lx", (long)pid->eventlist[i]);
	putchar('\n');
    }
    if (pid->wakevent>0)
...
Internal_Signal(char *event)
{
    ...
    Debug(0, ("Entered Internal_Signal [event id 0x%lx]", (long)event));
    if (lwp_init == NULL)
	return LWP_EINIT;
    if (event == NULL) 
